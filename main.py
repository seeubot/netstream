...
async def initialize_telegram_bot():
    ...
    try:
        ...
        if not domain:
            logger.error("❌ KOYEB_PUBLIC_DOMAIN environment variable not set.")
            return False
...
def create_app():
    ...
    def startup():
        ...
        # Initialize Telegram bot in a new event loop
        try:
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            success = loop.run_until_complete(initialize_telegram_bot())
            if not success:
...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=port, debug=False)FRONTEND_URL = os.getenv('FRONTEND_URL', 'https://your-frontend.vercel.app')
MAX_FILE_SIZE = 4000 * 1024 * 1024  # 4GB limit
